...
- 認証済みタイムラインは50件ずつ取得する。`date`、`created_at`、`id` のカーソルで次ページを取得するため、作成・削除中にも重複や欠落が起きにくい。
- ページ境界にあるペア記録は同時に補完して表示する。編集時に未読込の記録・ペア候補が必要になった場合はRLS下で追加取得する。
- 検索・統計は読み込み済みの記録だけを対象とし、追加ページがある場合はUIにその旨を表示する。
- 公開プロフィールはSSRを維持し、`?page=` で50件ずつ表示する。2ページ目以降は重複した検索結果を避けるため`noindex, follow`とし、ルートページをcanonicalとする。
- 公開プロフィールのdescriptionとOGPの記録数は、ページ内の取得件数ではなく総件数RPCで算出する。

## 今後の監視点

- ページサイズ50件でのSupabase REST応答時間、追加読込のp95、ペア補完クエリ数を継続監視する。
- 検索と統計を全件対象にする要件が生じた場合は、サーバー側フィルタと集計RPCを追加する。
